...
# --- ★★★ 修正: 全編解析関数 (v13.6 UnboundLocalError 修正) ★★★ ---
...
# --- メイン実行ブロック (v13.5 と変更なし) ---
if __name__ == "__main__":
    
    print("--- ワークフロー開始 (v13.6 Fix UnboundError) ---")
    ...
    num_groups = int(np.ceil(num_videos / ROI_GROUP_SIZE))
    print(f"\n--- {num_videos} 件の動画が検出されました ({num_groups} グループに分割, 1グループ {ROI_GROUP_SIZE} 件) ---")

    for i in range(0, num_videos, ROI_GROUP_SIZE):
        
        current_group_index = (i // ROI_GROUP_SIZE) + 1
        video_group = video_files[i : i + ROI_GROUP_SIZE]
        representative_video = video_group[0] 

        print("\n" + "="*70)
        print(f"--- グループ {current_group_index} / {num_groups} の共通ROIを選択します ---")
        print(f" (代表動画: {os.path.basename(representative_video)})")
        
        try:
            analysis_roi = select_analysis_roi(representative_video)
            
            if analysis_roi:
                print(f"--- グループ {current_group_index} の処理を開始します ({len(video_group)}件) ---")
                
                for j, video_path in enumerate(video_group):
                    print(f"\n--- 処理中 (グループ {current_group_index} - {j+1}/{len(video_group)}): {os.path.basename(video_path)} ---")
                    try:
                        analyze_video_file(
                            video_path, 
                            output_folder, 
                        ...
                        print(f"  [!!! 重大なエラー !!!] {os.path.basename(video_path)} の処理中にエラーが発生しました: {e}")
                        print("  -> このグループの次のファイルの処理に進みます...")

            else:
                print(f"  -> グループ {current_group_index} のROI選択がキャンセルされたため、このグループをスキップします。")

        except Exception as e:
            print(f"  [!!! 重大なエラー !!!] グループ {current_group_index} のROI選択中にエラーが発生しました: {e}")
            print("  -> 次のグループの処理に進みます...")
