...
plt.rcParams.update({
    'font.size': 18, 
    'axes.labelsize': 30,  # 軸タイトルサイズ (大きく)
    'xtick.labelsize': 20, # X軸目盛りサイズ (大きく)
    'ytick.labelsize': 20, # Y軸目盛りサイズ (大きく)
    'legend.fontsize': 20, # 凡例サイズ (大きく)
...
def analyze_single_file(force_path, tip_path, output_dir, a, b, col_c, col_d, fps, vel_thresh_per_frame, px_per_mm):
    """
    v28.5: 動的/静的判定の削除、単位をNとmmに修正、軸サイズ拡大
    """
    ...
    try:
        ...
        # ★★★ 修正: 把持力 [mN] を [N] に変換 (単位調整はグラフ表示時) ★★★
        force_mN = (a * x**2) + (b * x) 
        force_N = force_mN / 1000.0 # Nに変換
        ...
        # 2. 静的/動的 判定ロジックはグラフ描画から除外するため、計算も不要。
        # df_merged['is_static'] の列生成は行いません。
        
        # --- ステップ5: グラフを描画 (シェーディング削除 & 単位/サイズ修正) ---
        ...
        ax1.set_ylabel('Distance (mm)') # 単位を mm に修正
        ax1.grid(True, linestyle='--', alpha=0.5)
        
        y_min, y_max = ax1.get_ylim()
        
        # ★★★ 削除: 動的/静的のシェーディング fill_between を削除 ★★★
        
        ax1.set_ylim(y_min, y_max) 
        ax1.legend(loc='upper left')

        # --- 下段 (把持力グラフ) ---
        ax2.plot(time_axis, df_merged['force'], color='red', label='Gripping Force', lw=2)
        ax2.set_ylabel('Gripping Force (N)') # 単位を N に修正
        ax2.set_xlabel('Time (s)')
        ax2.grid(True, linestyle='--', alpha=0.5)
        
        y_min, y_max = ax2.get_ylim()
        ax2.set_ylim(0, max(y_max * 1.1, 0.1)) # 最小値を0に (最小値が0.1未満にならないように)
        ax2.legend(loc='upper right')
        
        plt.tight_layout()
        
        # グラフを保存
        output_path = os.path.join(output_dir, f"{base_name}_force_EFFECTIVE_distance_N_mm_graph.png") # ファイル名変更
...
if __name__ == "__main__":
    ...
    print("--- v28.5: 統合解析 (N・mm単位、シェーディング削除) ---")
